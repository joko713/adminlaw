...
(function addCases(){
  ...
  add('probuild', {
    ...
    en:{ facts:'NSW Building and Construction Industry Security of Payment Act privative provision; non-JE error of law on the face of the record.',
      ...
      principle:'Probuild clarified that State privative clauses CAN validly oust certiorari for non-jurisdictional errors of law on the face of the record — Kirk only protects review for jurisdictional error. So a State privative clause survives if it is read down to leave the entrenched JE minimum intact while excluding non-JE error. Essential to understanding the precise scope of the Kirk minimum: not every error of law gets the entrenched-minimum treatment.',
      when:'Cite when the alleged error in a State decision is non-jurisdictional (e.g. an error of law on the face of the record that does not go to jurisdiction) and the statute contains a privative clause — to show that the clause can validly exclude certiorari in that case, even after Kirk. Also use as the precise boundary marker for the Kirk entrenched minimum.',
    ...
    kr:{ facts:'NSW 건설업 결제 보안법의 privative 조항. 기록상 표면적 비-JE 법률 오류.',
      ...
      principle:'Probuild는 주(州) 배제조항이 기록상 표면의 비-관할권적(non-JE) 법률 오류에 대한 certiorari는 합법적으로 봉쇄할 수 있다고 정리한 판결. Kirk가 보호하는 것은 관할권적 오류에 대한 심사뿐이고, 비-JE 오류까지 entrenched minimum이 미치는 것은 아니다. 따라서 주 배제조항은 — JE entrenched minimum을 그대로 두면서 비-JE 오류만 배제한다고 해석되면 — 유효하다. Kirk 보호의 정확한 경계선을 가른 케이스.',
      when:'주 결정의 다투어진 오류가 비-JE(예: 관할권에 가지 않는, 기록 표면상 법률 오류)이고 그 법령에 배제조항이 있을 때 인용 — Kirk 이후에도 그런 경우에는 배제조항이 유효하게 certiorari를 막을 수 있음을 보이는 카드. Kirk entrenched minimum의 정확한 경계선으로도 쓴다.',
  ...
  add('abt17', {
    ...
    en:{ facts:'IAA decision-making procedure under Migration Act Pt 7AA.',
      ...
      principle:'ABT17 confirmed that the entrenched-minimum logic of Kirk and S157 binds the Commonwealth too at the boundary of what counts as a genuine review. Where a statutory scheme requires a real consideration of new information (as Part 7AA of the Migration Act did for the IAA), the failure to do so is jurisdictional error — and the entrenched minimum protects review of it. The case strengthens the convergence of State and Commonwealth entrenchment lines: at the JE floor, both jurisdictions look the same.',
      when:'Cite when a Commonwealth decision involves a statutory duty of genuine consideration of new information — particularly under a scheme that frames that consideration as a condition of the power. Pair with Probuild to show the State vs Commonwealth entrenched-minimum picture.',
    ...
    kr:{ facts:'Migration Act 제7AA부에 따른 IAA 결정 절차.',
      ...
      principle:'ABT17은 Kirk·S157의 entrenched minimum 논리가 연방 차원에서도 \"진정한 검토\"의 경계에서 작동함을 확인한 판결. 법령 체계가 진정한 검토(예: Migration Act 제7AA부 — IAA에 새 정보의 진정한 고려 요구)를 요구하는 경우, 이를 하지 않은 것은 관할권적 오류이고 entrenched minimum이 그 심사를 보호한다. Kirk 이후 entrenchment 라인이 더 강화되어 — JE 바닥선에서 주·연방이 수렴하는 흐름의 연방 측 신호다.',
      when:'연방 행정 결정에서 \"검토할 것을 진정으로 검토하지 않았다\"는 의무 위반이 쟁점일 때 — 특히 법령 체계 자체가 그 검토를 권한의 조건으로 만들었을 때 — 인용. Probuild와 함께 주 vs 연방 entrenched minimum 비교의 한 축으로 쓴다.',
  ...
  add('miah', {
    ...
    en:{ facts:'Migration code procedure said to exclude PF.',
      ...
      principle:'Miah reaffirmed the strong common-law presumption that the fair-hearing rule applies to administrative decision-making and is NOT excluded simply by codifying the procedure in the statute. The Migration Act procedural code did not by itself exclude PF — express, clear words are required (a proposition Saeed later restated). The leading authority for resisting an \"it is a code, therefore PF is excluded\" argument.',
      when:'Cite when the government argues that a statutory procedural \"code\" (e.g. Migration Act Pt 7) has excluded common-law PF. Miah + Saeed together require express, clear words to displace PF — codification alone is not enough.',
    ...
    kr:{ facts:'이민 절차 코드가 PF를 배제한다는 주장.',
      ...
      principle:'Miah는 절차적 공정(공정한 청문 규칙)이 행정 결정에 적용된다는 강한 보통법 추정을 — 단순한 \"코드화\"만으로는 — 배제되지 않는다고 재확인한 판결. 이민 절차를 법령으로 코드화한 것만으로 PF가 배제되지는 않으며 — 배제하려면 명시적·분명한 문언이 필요하다(Saeed가 한 번 더 못박았다). \"이건 코드이니 PF는 없다\"는 정부 측 주장에 정면으로 반박할 때 가장 먼저 꺼내는 판례.',
      when:'정부가 법령상 절차 \"코드\"(예: Migration Act Pt 7)가 보통법상 PF를 배제했다고 주장할 때 인용. Miah + Saeed는 함께 PF 배제에 명시적·분명한 문언을 요구한다 — 단순 코드화로는 부족.',
  ...
  add('szmta', {
    ...
    en:{ facts:'Notification under s 438 of Migration Act not given to tribunal.',
      ...
      principle:'SZMTA was the immediate predecessor to MZAPC and established materiality as a substantive threshold for procedural-fairness JE. A breach of a notification duty under s 438 of the Migration Act is jurisdictional only if it was material — only if there was a realistic possibility the decision could have been different had the breach not occurred. The case re-aligns PF doctrine with the Hossain materiality presumption: not every procedural failure invalidates.',
      when:'Cite when the alleged JE is a breach of a statutory notification/procedural duty (e.g. failure to disclose s 438 information in migration cases) — to show materiality must be argued separately. Pair with Hossain (the framework) and MZAPC (onus on applicant) for the full PF + materiality argument.',
    ...
    kr:{ facts:'Migration Act s 438 통지 의무 미준수.',
      ...
      principle:'SZMTA는 MZAPC의 직접 전신. 절차적 공정 위반의 JE 인정에도 materiality(중요성)가 실질적 문턱으로 적용된다는 점을 정립한 판결. Migration Act s 438 통지 의무 위반은 그 자체로 관할권적 오류가 아니라 — 위반이 없었다면 결정이 달라졌을 현실적 가능성이 있었어야 — 관할권적이다. Hossain의 materiality 추정을 PF 영역에 맞춰 재배치한 판결로, 모든 절차 위반이 무효를 부르지는 않는다는 점을 강조.',
      when:'다투어진 JE가 법령상 통지·절차 의무 위반일 때 인용 — 예컨대 이민 사안에서 s 438 정보 미공개. materiality를 별도로 다투어야 한다는 점을 보이는 판례. Hossain(분석틀)과 MZAPC(원고 입증책임)와 함께 PF + materiality 논거 완성.',
